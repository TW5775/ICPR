...
class augumentation(object):
    def __call__(self, input, target):
        ...
        input = input * brightness_factor
        target = target * brightness_factor
        input = gaussian_filter(input, sigma=0.5)  # 替换合适的sigma值，控制滤波程度
        target = gaussian_filter(target, sigma=0.5)  # 目标数据也进行滤波，可根据需要调整sigma
        return input, target
