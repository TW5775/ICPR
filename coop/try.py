...
def train_and_tune(trial):
    opt.model_name = trial.suggest_categorical('model_names', ['ACM', 'ALCNet', 'DNANet', 'ISTDU-Net', 'RDIAN'])
    opt.dataset_name = 'WideIRSTD'
...
def test(save_pth, trial, idx_epoch):
    ...
    global mIoU_value
    pixAcc, mIoU_value = eval_mIoU.get()
    if (idx_epoch + 1) == opt.nEpochs and (idx_epoch + 1) % 50 != 0:
        trial.report(mIoU_value, step=1)
...
if __name__ == '__main__':
    ...
    study.optimize(train_and_tune, n_trials=5)  # 替换100为你希望的试验次数
    # 保存平行坐标图
    plt.figure()
    optvis.plot_parallel_coordinate(study)
